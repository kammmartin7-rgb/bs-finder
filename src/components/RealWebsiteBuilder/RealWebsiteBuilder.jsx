...
function SitePreview({ customer, site, viewport = 'desktop', full = false, onClose }) {
  ...
  const isHebrew = customer.language === 'he'
  const phone = String(customer.phone || '').replace(/[^+\d]/g, ''); const whatsapp = String(customer.whatsapp || customer.phone || '').replace(/\D/g, '')
  const maps = customer.address ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(customer.address)}` : ''
...
export default function RealWebsiteBuilder({ initialLead }) {
  ...
  return <section className="real-builder"><header><div><span>Business OS</span><h1>{t('realWebsiteBuilder')}</h1><p>{t('rwbSubtitle')}</p></div><div className="real-builder__header-actions"><button type="button" onClick={() => { setSite(liveSite); setNotice(t('rwbCreated')) }}>{t('rwbCreate')}</button><button type="button" onClick={() => persist('draft')}>{t('rwbSaveDraft')}</button><button type="button" onClick={() => persist('ready-to-publish')}>{t('rwbMarkReady')}</button></div></header>
    <div className="real-builder__body"><form className="real-builder__form" onSubmit={(event) => event.preventDefault()}><div className="real-builder__form-head"><h2>{t('rwbCustomerDetails')}</h2><select value={projectMeta.status} onChange={(event) => setProjectMeta((current) => ({ ...current, status: event.target.value }))}>{STATUSES.map((status) => <option key={status} value={status}>{t(`rwbStatus_${status}`)}</option>)}</select></div>
      ...
      <aside className="real-builder__projects"><h2>{t('rwbProjects')}</h2><div>{projects.length ? projects.map((project) => <article key={project.id}><div><strong>{project.customer.businessName}</strong><small>{project.customer.language.toUpperCase()} · {t(REAL_WEBSITE_TEMPLATES.find((item) => item.id === project.selectedTemplate)?.nameKey || 'rwbTemplateModern')}</small><small>{t(`rwbStatus_${project.status}`)} · {new Date(project.updatedDate).toLocaleDateString()}</small></div><button type="button" onClick={() => loadProject(project)}>{t('rwbEdit')}</button><button type="button" onClick={() => { loadProject(project); setFullPreview(true) }}>{t('rwbPreview')}</button></article>) : <p>{t('rwbNoProjects')}</p>}</div></aside>
    </div>{fullPreview && <SitePreview customer={customer} site={activeSite} full onClose={() => setFullPreview(false)} />}</section>
}
